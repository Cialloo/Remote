...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COPYDATA:
	{
		...
		pPkt->lengt = strlen(pStr) + 1;//仅仅表示数据长度，不过包括结构体头部的前面2个4字节
		memcpy(pPkt->data, pStr, pPkt->lengt);//将数据拷贝给包中
		send(s,
			(const char*)pPkt,
			pPkt->lengt + sizeof(unsigned int) * 2,
			0);
